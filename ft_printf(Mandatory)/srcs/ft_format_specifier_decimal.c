...
/*   Updated: 2023/04/30 23:18:28 by jintan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char* ft_int_to_str(int* num)
{
    ...
    else
    {
        ...
        {
            *num = -*num;
            char* str = ft_itoa_base(*num, BASE_10);
            char* result = ft_strjoin("-", str);
            free(str);
            return (result) ;
        }
        else
...
{
    char* str = ft_int_to_str(&value);
    int c = ft_format_specifier_string(str);
    free(str);
    return c;
}
