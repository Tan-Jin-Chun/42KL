...
/*   Updated: 2023/04/30 23:15:59 by jintan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_format(int i, va_list args)
{
	...
	if (i == 'c')
		...
		c = ft_format_specifier_hexadecimal_lowercase(va_arg(args, unsigned long));
	else if (i == 'X')
		c = ft_format_specifier_hexadecimal_uppercase(va_arg(args, unsigned long));
	else if (i == '%')
...
/*
Function:
The function loops through each character of the input string str, checks if it is 
...
/*
Function:
Format and print output to standard output using a variable number of arguments
and a format string.
...
It takes a constant character pointer,format as the format string, and a 
variable number of arguments indicated by the ellipsis ...
...
str is assigned the result of ft_strdup(format), which duplicates the format string. 
If the duplication fails and str is NULL, the function returns -1, indicating an error.

4. Initialize the variadic arguments
va_start(args, format) initializes the args variable to handle the variable number 
of arguments that follow the format string.

5. Parse the input and print formatted output
The ft_parse_input function is called with the duplicated format string str 
and the args variable.
This function is responsible for parsing the input and printing the formatted output. 
The number of characters printed is added to the counter.

6. Clean up
va_end(args) is called to clean up the memory allocated for the variadic arguments. 
The memory allocated for the duplicated format string is freed using free((void*)str).
